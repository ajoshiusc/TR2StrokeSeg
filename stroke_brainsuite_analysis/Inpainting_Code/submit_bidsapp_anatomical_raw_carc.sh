...
run_subject_worker() {
    ...
    local subject_input="$raw_root/$subject"
    local subject_output="$output_root/$subject"
    [[ -d "$subject_input" ]] || die "subject directory is missing: $subject_input"
    ...
    mapfile -d '' brains < <(
        find "$subject_input" -mindepth 2 -maxdepth 3 -type l \
            -name '*_T1w.nii.gz' -print0 | sort -z
    )
    if ((${#brains[@]} == 0)); then
    ...
    # Fallback to standard .nii
    if ((${#brains[@]} == 0)); then
        mapfile -d '' brains < <(
            find "$subject_input" -mindepth 2 -maxdepth 3 -type f \
                -name '*_T1w.nii' -print0 | sort -z
        )
    fi
    ((${#brains[@]} > 0)) || die "no T1 scans found for $subject"

    local -a complete_brains=()
    for brain in "${brains[@]}"; do
        complete_brains+=("$brain")
    done
    ((${#complete_brains[@]} > 0)) || die "no T1 scans found for $subject"

    export OMP_NUM_THREADS="${SLURM_CPUS_PER_TASK:-1}"

    local completed=0
    for brain in "${complete_brains[@]}"; do
        local source_dir brain_name case_id session full_t1 case_output staged_t1 ext
        source_dir="$(dirname -- "$brain")"
        brain_name="$(basename -- "$brain")"
        
        # Handle both .nii.gz and .nii
        if [[ "$brain_name" == *.nii.gz ]]; then
            case_id="${brain_name%_T1w.nii.gz}"
            ext=".nii.gz"
        else
            case_id="${brain_name%_T1w.nii}"
            ext=".nii"
        fi
        ...
        mkdir -p "$case_output"
        staged_t1="$case_output/${case_id}_T1w${ext}"
...
usage() {
    cat <<EOF
Usage: $0 [OPTIONS] RAW_DIR OUTPUT_DIR [BRAINSUITE_ARGS...]

Submit CARC jobs to run the BrainSuite BIDS App anatomical pipeline on original
T1 subjects.

Arguments:
  RAW_DIR       Input directory containing sub-*/ses-*/... directories.
...
Examples:
  $0 . data/ARC data/ARC/derivatives/brainsuite_anatomical_raw
EOF
}
...
cd "$raw_dir" || die "failed to enter $raw_dir"

readarray -t subjects < <(find . -maxdepth 1 -mindepth 1 -type d -name 'sub-*' | \
    sed 's|^\./||' | sort)

((${#subjects[@]} > 0)) || die "no sub-* directories found in $raw_dir"
...
for sub in "${subjects[@]}"; do
    ...
    wrap_cmd=(bash "$SCRIPT_PATH" --worker "$raw_dir" "$output_dir" "$sub" -- "$@")
