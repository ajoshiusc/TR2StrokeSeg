...
\input{analysis/arc_inpainting/paper_results.tex}
\newif\ifAQResultsAvailable
\IfFileExists{analysis/miccai_workshop/aq_results.tex}{%
  \AQResultsAvailabletrue
  \input{analysis/miccai_workshop/aq_results.tex}
}{\AQResultsAvailablefalse}
...
volume and language-network damage. Cohort-scale execution of this secondary
analysis is ongoing; we therefore report its complete protocol and a
representative technical result without making a predictive-superiority claim.
...
prediction comparison. We report completed cohort findings for Studies~1--2,
the implemented and locally verified outputs for Studies~3--4, and an explicit
analysis boundary for results still awaiting cohort-scale execution.
...
\subsection{Status of the incremental AQ study}
\ifAQResultsAvailable
The verified cohort analysis included \AQAnalysisN{} participants. Mean
repeated outer-CV MAE was \AQStandardMAE{} AQ points for the conventional
lesion model and \AQDeformationMAE{} after adding deformation. The paired MAE
advantage was \AQDeformationAdvantage{} points (participant-bootstrap 95\% CI
\AQDeformationAdvantageCI; Holm-adjusted $p=\AQDeformationP$).
\else
The cohort-wide jobs were incomplete at manuscript generation; no uncertainty
or deformation superiority result is reported. Code requires at least 90\%
uncertainty coverage, applies identical cohort filtering, and writes this
paragraph's values only after repeated nested cross-validation. Thus, the
representative case cannot be mistaken for a cohort result.
\fi
...
expected-volume calculations. Yet a softmax score is not a statistical
$p$-value and its entropy is not calibrated uncertainty. Manual ARC masks are
needed for calibration, Brier score, and threshold-conditioned Dice; ensembles
would characterize model uncertainty more directly \cite{Mehrtash2020}.
...
The AQ experiment is incremental, not causal. Including lesion volume and
language-network burden prevents deformation from receiving credit merely as a
surrogate of size or side; nested preprocessing prevents leakage. Any advantage
must survive fold variation and registration-sensitivity adjustment before
external validation.
...
Only one generative sample was used, scores are uncalibrated, and cohort-scale
uncertainty/deformation results remain pending. Priorities are non-inpainted and
normative controls, tissue-specific GM/WM summaries, calibration, and external
AQ validation.
...
treating synthetic anatomy or cross-sectional deformation as ground truth.
